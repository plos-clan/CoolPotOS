...
static void AcpiPowerInit() {
    ...
    if (PM1b_ENABLE_REG == len) PM1b_ENABLE_REG = 0;

    io_out16((int)PM1a_ENABLE_REG, (1 << 8));
    if (PM1b_ENABLE_REG) { io_out16((uint16_t)PM1b_ENABLE_REG, (uint8_t)(1 << 8)); }
...
static unsigned int acpi_find_table(char *Signature) {
    ...
    for (len = *((uint32_t *)(rsdt_t + 4)), ptr2 = rsdt_t + 36; ptr2 < rsdt_t + len;
        ...
        ptr = (uint8_t *)(uintptr_t)(rsdt_t[0] == 'X' ? *((uint64_t *)ptr2) : *((uint32_t *)ptr2));
    ...
    while (entrys--) {
        ...
        if (!AcpiCheckHeader(*p, "FACP")) {
            ...
            if (!AcpiCheckHeader(facp->DSDT, "DSDT")) {
                ...
                for (uint32_t i  = (uint32_t)S5Addr; i < ((uint32_t)S5Addr) + dsdtlen;
                     i          += PAGE_SIZE) {
...
int acpi_enable() {
    ...
    if (SMI_CMD && ACPI_ENABLE) {
        ...
        io_out8((uint16_t)SMI_CMD, ACPI_ENABLE);

        // wait enable
        for (i = 0; i < 300; i++) {
            if (io_in16((uint32_t)PM1a_CNT) & SCI_EN) { break; }
            clock_sleep(5);
        }
        // wait enable
        if (PM1b_CNT) {
            for (i = 0; i < 300; i++) {
                if (io_in16((uint32_t)PM1b_CNT) & SCI_EN) { break; }
                clock_sleep(5);
            }
        }
        // check enable status
...
int acpi_disable() {
    ...
    if (!io_in16((uint16_t)PM1a_CNT) & SCI_EN) return 0;

    if (SMI_CMD || ACPI_DISABLE) {
        io_out8((uint16_t)SMI_CMD, ACPI_DISABLE);

        for (i = 0; i < 300; i++) {
            if (!io_in16((uint16_t)PM1a_CNT) & SCI_EN) break;
            clock_sleep(5);
        }

        for (i = 0; i < 300; i++) {
            if (!io_in16((uint16_t)PM1b_CNT) & SCI_EN) break;
            clock_sleep(5);
        }
...
uint8_t *AcpiGetRSDPtr() {
    ...
    for (addr  = (uint32_t *)0x000E0000; addr < (uint32_t *)0x00100000;
...
uint8_t *AcpiCheckRSDPtr(void *ptr) {
    ...
    if (!memcmp(sign, bptr, 8)) {
        ...
        if (!check) { return (uint8_t *)rsdp->rsdt; }
    }
    return NULL;
}
...
void hpet_initialize() {
    ...
    if (!hpet) { dlogf("\033[31mCannot found acpi hpet table\033[0m\n"); }
