...
static int find_cmdslot(HBA_PORT *port) {
    ...
    dlogf("Cannot find free command list entry\n");
    return -1;
}
...
void ahci_init() {
    ...
    if (device == NULL) {
        ...
        klogf(true, "Find Serial ATA Controller.\n");
    ...
    for (i = 0; i < port_total; i++) {
        ...
        dlogf("SATA %d Found %s Drive %dMB - ", i,
