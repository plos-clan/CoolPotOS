...
static bool epollfs_close(void *current) {
    ...
    if (!ep) return true;
    if (ep->node) ep->node->handle = NULL;
    free(ep);
    return true;
}
...
static int epollfs_poll(void *file, size_t events) {
    ...
    for (int i = 0; i < ep->count; i++) {
        ...
        if (fs_callbacks[node->fsid]->poll == (void *)dummy) {
            if (events & EPOLLIN) { out |= EPOLLIN; break; }
            continue;
        }
        int revents = vfs_poll(node, ep->entries[i].events);
        if (revents > 0) {
            if (events & EPOLLIN) out |= EPOLLIN;
            break;
        }
    }
    spin_unlock(ep->lock);
...
void epollfs_regist() {
    ...
    if (epollfs_id == -EINVAL) { kerror("epollfs regist error."); }
...
syscall_(epoll_create1, int flags) {
    ...
    if (!epollfs_root) return SYSCALL_FAULT_(ENOMEM);

    epoll_instance_t *ep = calloc(1, sizeof(epoll_instance_t));
    if (!ep) return SYSCALL_FAULT_(ENOMEM);
...
syscall_(epoll_ctl, int epfd, int op, int fd, struct epoll_event *event) {
    ...
    if (!ep) return SYSCALL_FAULT_(EBADF);

    // Validate target fd exists
    fd_t *target = get_fd(fdt, fd);
    if (!target) return SYSCALL_FAULT_(EBADF);

    // Cannot add epoll fd to itself
    if (fd == epfd) return SYSCALL_FAULT_(EINVAL);
...
static bool eventfdfs_close(void *current) {
    ...
    if (!ctx) return true;
    if (ctx->node) ctx->node->handle = NULL;
    free(ctx);
    return true;
}
...
void eventfdfs_regist() {
    ...
    if (eventfdfs_id == -EINVAL) { kerror("eventfdfs regist error."); }
...
syscall_(eventfd2, uint64_t initval, int flags) {
    if (flags & ~(EFD_CLOEXEC | EFD_NONBLOCK | EFD_SEMAPHORE))
        return SYSCALL_FAULT_(EINVAL);
    if (!eventfdfs_root) return SYSCALL_FAULT_(ENOMEM);

    eventfd_ctx_t *ctx = calloc(1, sizeof(eventfd_ctx_t));
    if (!ctx) return SYSCALL_FAULT_(ENOMEM);
    ...
    handle->offset = 0;
    handle->flags  = 0;
    if (flags & EFD_CLOEXEC)  handle->flags |= O_CLOEXEC;
    if (flags & EFD_NONBLOCK) handle->flags |= O_NONBLOCK;
