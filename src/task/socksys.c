...
static int sock_create_fd(socket_info_t *info, int flags) {
    ...
    handle->offset = 0;
    handle->flags  = 0;
    if (flags & SOCK_NONBLOCK) handle->flags |= O_NONBLOCK;
    if (flags & SOCK_CLOEXEC) handle->flags |= O_CLOEXEC;
...
static socket_specific_t *get_sock_spec(int fd) {
    ...
    if (!(handle->node->type & file_socket)) return NULL;
    return (socket_specific_t *)handle->node->handle;
}

syscall_(socket, int domain, int type, int protocol) {
    if (domain != AF_UNIX) return SYSCALL_FAULT_(EAFNOSUPPORT);
    ...
    if (sockfs_root == NULL) return SYSCALL_FAULT_(ENOSYS);

    socket_info_t *info = alloc_socket_info(domain, type, protocol);
    if (!info) return SYSCALL_FAULT_(ENOMEM);
...
syscall_(socketpair, int domain, int type, int protocol, int *sv) {
    if (!sv) return SYSCALL_FAULT_(EINVAL);
    if (domain != AF_UNIX) return SYSCALL_FAULT_(EAFNOSUPPORT);
    int real_type = type & ~(SOCK_NONBLOCK | SOCK_CLOEXEC);
    if (real_type != SOCK_STREAM && real_type != SOCK_DGRAM)
        return SYSCALL_FAULT_(EPROTOTYPE);
    ...
    if (!info0 || !info1) {
        if (info0) { ringbuf_destroy(&info0->recv_buf); free(info0); }
        if (info1) { ringbuf_destroy(&info1->recv_buf); free(info1); }
        return SYSCALL_FAULT_(ENOMEM);
    }
...
syscall_(bind, int sockfd, struct sockaddr *addr, uint64_t addrlen) {
    ...
    if (!spec) return SYSCALL_FAULT_(ENOTSOCK);
    socket_info_t *info = spec->info;
    if (!info) return SYSCALL_FAULT_(ENOTSOCK);

    if (addr->sa_family != AF_UNIX) return SYSCALL_FAULT_(EAFNOSUPPORT);
...
syscall_(listen, int sockfd, int backlog) {
    ...
    socket_info_t *info = spec->info;
    if (!info) return SYSCALL_FAULT_(ENOTSOCK);
    if (!info->is_bound) return SYSCALL_FAULT_(EINVAL);
    if (info->type != SOCK_STREAM) return SYSCALL_FAULT_(EOPNOTSUPP);
...
syscall_(accept, int sockfd, struct sockaddr *addr, uint64_t *addrlen) {
    ...
    socket_info_t *info = spec->info;
    if (!info) return SYSCALL_FAULT_(ENOTSOCK);
    if (info->state != SS_LISTENING) return SYSCALL_FAULT_(EINVAL);
    ...
    for (;;) {
        ...
        if (info->pending_count > 0) {
            ...
            socket_info_t *server_info = alloc_socket_info(
                info->domain, info->type, info->protocol);
            if (!server_info) return SYSCALL_FAULT_(ENOMEM);
            ...
            if (new_fd < 0) return SYSCALL_FAULT_(ENOMEM);
            ...
            if (addr && addrlen) {
                ...
                if (client_info->is_bound)
                    strncpy(sun->sun_path, client_info->bound_path,
                            UNIX_PATH_MAX - 1);
...
syscall_(connect, int sockfd, struct sockaddr *addr, uint64_t addrlen) {
    ...
    if (info->type == SOCK_DGRAM) {
        ...
        socket_specific_t *target_spec =
            (socket_specific_t *)target_node->handle;
        if (!target_spec) return SYSCALL_FAULT_(ECONNREFUSED);
    ...
    // SOCK_STREAM connect
    if (info->state == SS_CONNECTED) return SYSCALL_FAULT_(EISCONN);
    if (info->state == SS_CONNECTING) return SYSCALL_FAULT_(EALREADY);
    ...
    socket_specific_t *server_spec =
        (socket_specific_t *)target_node->handle;
    if (!server_spec) return SYSCALL_FAULT_(ECONNREFUSED);
...
syscall_(shutdown, int sockfd, int how) {
    socket_specific_t *spec = get_sock_spec(sockfd);
    if (!spec) return SYSCALL_FAULT_(ENOTSOCK);
    if (how == SHUT_RD || how == SHUT_RDWR) spec->shut_rd = true;
    if (how == SHUT_WR || how == SHUT_RDWR) spec->shut_wr = true;
    return EOK;
}
...
syscall_(sendto, int sockfd, void *buf, size_t len, int flags,
    ...
    socket_info_t *info = spec->info;
    if (!info) return SYSCALL_FAULT_(ENOTSOCK);
    if (spec->shut_wr) return SYSCALL_FAULT_(EPIPE);
        ...
        if (sun->sun_family == AF_UNIX && sun->sun_path[0] != '\0') {
            ...
            if (path) {
                ...
                if (tnode && (tnode->type & file_socket)) {
                    socket_specific_t *ts =
                        (socket_specific_t *)tnode->handle;
                    if (ts) target = ts->info;
                }
            }
        }
    }

    if (!target) return SYSCALL_FAULT_(ENOTCONN);
    ...
    while (remaining > 0) {
        ...
        if (avail > 0) {
            ...
            if (info->type == SOCK_DGRAM) break; // DGRAM: single write
        } else {
            spin_unlock(target->lock);
...
syscall_(recvfrom, int sockfd, void *buf, size_t len, int flags,
    ...
    for (;;) {
        ...
        spin_unlock(info->lock);
        if (flags & MSG_DONTWAIT) return SYSCALL_FAULT_(EAGAIN);
        scheduler_yield();
    }
}

syscall_(sendmsg, int sockfd, struct msghdr *msg, int flags) {
    if (!msg) return SYSCALL_FAULT_(EINVAL);
    size_t total = 0;
    for (size_t i = 0; i < msg->msg_iovlen; i++) {
        struct iovec *iov = &msg->msg_iov[i];
        if (iov->iov_len == 0) continue;
        size_t ret = syscall_sendto(sockfd, iov->iov_base, iov->iov_len,
                                    flags, msg->msg_name, msg->msg_namelen,
                                    regs);
        if ((int64_t)ret < 0) return total > 0 ? total : ret;
        total += ret;
    }
    return total;
}
...
syscall_(recvmsg, int sockfd, struct msghdr *msg, int flags) {
    ...
    for (size_t i = 0; i < msg->msg_iovlen; i++) {
        struct iovec *iov = &msg->msg_iov[i];
        if (iov->iov_len == 0) continue;
        size_t ret = syscall_recvfrom(sockfd, iov->iov_base, iov->iov_len,
                                      flags, NULL, NULL, regs);
        if ((int64_t)ret < 0) return total > 0 ? total : ret;
        total += ret;
        if (ret < iov->iov_len) break; // short read
    }
...
syscall_(getsockopt, int sockfd, int level, int optname, void *optval,
    ...
    socket_info_t *info = spec->info;
    if (!info) return SYSCALL_FAULT_(ENOTSOCK);
    if (!optval || !optlen) return SYSCALL_FAULT_(EINVAL);
    ...
    if (level == SOL_SOCKET) {
        ...
        case SO_ERROR:
            ...
            info->so_error = 0;
            break;
        case SO_TYPE: val = info->type; break;
        case SO_DOMAIN: val = info->domain; break;
        case SO_RCVBUF: val = (int)info->recv_buf.capacity; break;
        case SO_SNDBUF: val = SOCK_BUFF; break;
        default: return EOK;
        }
...
syscall_(getsockname, int sockfd, struct sockaddr *addr, uint64_t *addrlen) {
    ...
    socket_info_t *info = spec->info;
    if (!info) return SYSCALL_FAULT_(ENOTSOCK);
    if (!addr || !addrlen) return SYSCALL_FAULT_(EINVAL);
...
syscall_(getpeername, int sockfd, struct sockaddr *addr, uint64_t *addrlen) {
    ...
    if (!info) return SYSCALL_FAULT_(ENOTSOCK);
    if (!addr || !addrlen) return SYSCALL_FAULT_(EINVAL);
    if (!info->peer) return SYSCALL_FAULT_(ENOTCONN);
