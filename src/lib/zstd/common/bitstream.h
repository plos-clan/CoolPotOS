 ...
 *  Hence keep track how many bits are potentially stored into local register to avoid register
 * overflow. After a flushBits, a maximum of 7 bits might still be stored into local register.
 *
 *  Avoid storing elements of more than 24 bits if you want compatibility with 32-bits bitstream
 * readers.
 *
 ...
 *  A reload guarantee a minimum of ((8*sizeof(bitD->bitContainer))-7) bits when its result is
 * BIT_DStream_unfinished. Otherwise, it can be less than that, so proceed accordingly. Checking if
 * DStream has reached its end can be performed with BIT_endOfDStream().
 */
...
MEM_STATIC FORCE_INLINE_ATTR size_t
 ...
 *           when status == BIT_DStream_unfinished, internal register is filled with at least 25 or
 * 57 bits */
MEM_STATIC BIT_DStream_status BIT_reloadDStream(BIT_DStream_t *bitD) {
    ...
    /* start < ptr < limitPtr */
    {
        ...
        bitD->bitContainer =
            MEM_readLEST(bitD->ptr); /* reminder : srcSize > sizeof(bitD->bitContainer), otherwise
                                        bitD->ptr == bitD->start */
        return result;
    }
}
