 ...
 * (dt is large enough, normalizedCounter distribution total is a power of 2, max is within range,
 * etc.) in which case it cannot fail. The workspace must be 4-byte aligned and at least
 * ZSTD_BUILD_FSE_TABLE_WKSP_SIZE bytes, which is defined in zstd_decompress_internal.h. Internal
 * use only.
 */
