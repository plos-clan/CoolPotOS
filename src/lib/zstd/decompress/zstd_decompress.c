...
size_t ZSTD_getFrameHeader_advanced(
    ...
    ZSTD_memset(
        zfhPtr, 0, sizeof(*zfhPtr)
    ); /* not strictly necessary, but static analyzer do not understand that zfhPtr is only going to
          be read only if return value is zero, since they are 2 different signals */
...
size_t ZSTD_decompressContinue(
    ...
    case ZSTDds_decodeSkippableHeader:
        ...
        dctx->expected = MEM_readLE32(
            dctx->headerBuffer + ZSTD_FRAMEIDSIZE
...
ZSTD_loadDEntropy(ZSTD_entropyDTables_t *entropy, const void *const dict, size_t const dictSize) {
...

    {
        ...
        size_t const litlengthHeaderSize = FSE_readNCount(
            litlengthNCount, &litlengthMaxValue, &litlengthLog, dictPtr, (size_t)(dictEnd - dictPtr)
        );
