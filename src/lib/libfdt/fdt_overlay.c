...
int fdt_overlay_target_offset(const void *fdt, const void *fdto, int fragment_offset,
                              char const **pathp) {
    uint32_t    phandle;
...
static int overlay_phandle_add_offset(void *fdt, int node, const char *name, uint32_t delta) {
    ...
    val = fdt32_ld(valp);
    if (val + delta < val || val + delta == (uint32_t)-1) return -FDT_ERR_NOPHANDLES;
 ...
 *      Negative error code on failure
 */
static int overlay_fixup_one_phandle(void *fdt, void *fdto, int symbols_off, const char *path,
                                     uint32_t path_len, const char *name, uint32_t name_len,
...
static int overlay_fixup_phandle(void *fdt, void *fdto, int symbols_off, int property) {
    ...
    do {
        ...
        ret = overlay_fixup_one_phandle(fdt, fdto, symbols_off, path, path_len, name, name_len,
                                        poffset, phandle);
        if (ret) return ret;
    } while (len > 0);
 ...
 *      Negative error code on failure
 */
static int overlay_update_node_conflicting_references(void *fdto, int tree_node, int fixup_node,
                                                      uint32_t fdt_phandle, uint32_t fdto_phandle) {
    int fixup_prop;
    int fixup_child;
    int ret;
    ...
    fdt_for_each_property_offset(fixup_prop, fdto, fixup_node) {
        ...
        for (i = 0; i < fixup_len; i++) {
            ...
            if (valp == fdto_phandle) fdt32_st(refp, fdt_phandle);
        }
    }
    ...
    fdt_for_each_subnode(fixup_child, fdto, fixup_node) {
        ...
        ret = overlay_update_node_conflicting_references(fdto, tree_child, fixup_child, fdt_phandle,
                                                         fdto_phandle);
        if (ret) return ret;
    }
 ...
 *      Negative error code on failure
 */
static int overlay_update_local_conflicting_references(void *fdto, uint32_t fdt_phandle,
    ...
    return overlay_update_node_conflicting_references(fdto, 0, fixups, fdt_phandle, fdto_phandle);
}
 ...
 *      Negative error code on failure
 */
static int overlay_prevent_phandle_overwrite_node(void *fdt, int fdtnode, void *fdto,
                                                  int fdtonode) {
    ...
    if (fdt_phandle && fdto_phandle) {
        ...
        ret = overlay_adjust_local_conflicting_phandle(fdto, fdtonode, fdt_phandle);
        if (ret) return ret;

        ret = overlay_update_local_conflicting_references(fdto, fdt_phandle, fdto_phandle);
        if (ret) return ret;
    }
    ...
    fdt_for_each_subnode(fdtochild, fdto, fdtonode) {
        ...
        ret = overlay_prevent_phandle_overwrite_node(fdt, fdtchild, fdto, fdtochild);
        if (ret) return ret;
    }
...
static int overlay_prevent_phandle_overwrite(void *fdt, void *fdto) {
    ...
    fdt_for_each_subnode(fragment, fdto, 0) {
        ...
        ret = overlay_prevent_phandle_overwrite_node(fdt, target, fdto, overlay);
        if (ret) return ret;
    }
