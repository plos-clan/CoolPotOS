...
static const struct fdt_property *fdt_get_property_namelen_(
    const void *fdt, int offset, const char *name, int namelen, int *lenp, int *poffset
) {
...
fdt_get_property_namelen(const void *fdt, int offset, const char *name, int namelen, int *lenp) {
    /* Prior to version 16, properties may need realignment
     * and this API does not work. fdt_getprop_*() will, however. */
    if (!can_assume(LATEST) && fdt_version(fdt) < 0x10) {
        if (lenp)
...
fdt_getprop_namelen(const void *fdt, int nodeoffset, const char *name, int namelen, int *lenp) {
    ...
    /* Handle realignment */
    if (!can_assume(LATEST) && fdt_version(fdt) < 0x10 && (poffset + sizeof(*prop)) % 8
...
const void *fdt_getprop_by_offset(const void *fdt, int offset, const char **namep, int *lenp) {
    ...
    /* Handle realignment */
    if (!can_assume(LATEST) && fdt_version(fdt) < 0x10 && (offset + sizeof(*prop)) % 8
        && fdt32_ld_(&prop->len) >= 8)
        return prop->data + 4;
    return prop->data;
}
...
static const void *fdt_path_getprop_namelen(
    const void *fdt, const char *path, const char *propname, int propnamelen, int *lenp
) {
...
const char *fdt_get_alias_namelen(const void *fdt, const char *name, int namelen) {
    ...
    if (!can_assume(VALID_DTB) && !(alias && len > 0 && alias[len - 1] == '\0' && *alias == '/'))
        return NULL;
...
const char *
fdt_stringlist_get(const void *fdt, int nodeoffset, const char *property, int idx, int *lenp) {
    const char *list, *end;
    int length;
