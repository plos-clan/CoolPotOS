...
static pte_t *walk_page_table(pte_t *table, uintptr_t va, int level, int huge_level, int alloc) {
...
static int map_4k_page(pte_t *root_table, uintptr_t va, uintptr_t pa, uint64_t perm) {
...
static int map_2m_page(pte_t *root_table, uintptr_t va, uintptr_t pa, uint64_t perm) {
...
static int map_1g_page(pte_t *root_table, uintptr_t va, uintptr_t pa, uint64_t perm) {
...
static int map_region(pte_t *root_table, uintptr_t pa_start, uintptr_t pa_end, uint64_t perm) {
    ...
    while (current < end) {
        ...
        if (ALIGN_UP(current, SIZE_1G) == current && remaining >= SIZE_1G) {
            if (map_1g_page(root_table, current + KERNEL_VIRTUAL_BASE, current, perm) < 0) {
                return -1;
            }
            current += SIZE_1G;
            continue;
        ...
        if (ALIGN_UP(current, SIZE_2M) == current && remaining >= SIZE_2M) {
            if (map_2m_page(root_table, current + KERNEL_VIRTUAL_BASE, current, perm) < 0) {
                return -1;
            }
            current += SIZE_2M;
            continue;
        ...
        // 最后使用4KB页
        if (map_4k_page(root_table, current + KERNEL_VIRTUAL_BASE, current, perm) < 0) {
            return -1;
        }
        current += SIZE_4K;
    }
