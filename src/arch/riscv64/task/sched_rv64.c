...
_Noreturn void arch_switch_to_user_mode() {
    ...
    thread->context.user_stack_top = thread->context.user_stack + BIG_USER_STACK;
    ...
    vma_t *region = vma_find_intersection(
        &thread->process->vma_manager, thread->context.user_stack, thread->context.user_stack_top);
    if (!region) {
