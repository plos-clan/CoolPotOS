...
uint64_t thread_clone(
    ...
    if (flags & CLONE_PARENT_SETTID) {
        *parent_tid = new_task->tid;
    }

    if (flags & CLONE_CHILD_SETTID) {
        *child_tid = new_task->tid;
    }
