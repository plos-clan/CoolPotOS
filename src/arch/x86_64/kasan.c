...
static int kasan_add_range(uintptr_t start, uintptr_t end, bool poison_initial) {
    if (start >= end || kasan_range_count >= KASAN_MAX_RANGES) return -1;
...
static void kasan_report(const char *reason, uintptr_t bad, size_t size, bool is_write,
                         uint8_t shadow_val) {
    kasan_disable_depth++;
    logkf("\n[KASAN] INVALID %s access\n", is_write ? "WRITE" : "READ");
    logkf("[KASAN] addr=%p size=%llu shadow=0x%02X\n", (void *)bad, (unsigned long long)size,
          shadow_val);
    if (reason) logkf("[KASAN] from=%s\n", reason);
...
static void kasan_check_range_in_range(const kasan_range_t *range, uintptr_t start,
                                       uintptr_t end, bool is_write, const char *reason) {
    uintptr_t cur = start;
    while (cur < end) {
...
void kasan_pop_disable(void) {
    if (kasan_disable_depth > 0) kasan_disable_depth--;
}

void kasan_heap_init(uintptr_t heap_start, size_t heap_size) {
    if (!kasan_initialized) kasan_init();
    if (kasan_heap_range >= 0 || heap_size == 0) return;
...
void kasan_heap_extend(uintptr_t heap_start, size_t heap_size) {
    if (!kasan_initialized || heap_size == 0) return;
    ...
    kasan_memset_u8((uint8_t *)(range->shadow_base + range->shadow_size), KASAN_POISON,
...
void kasan_poison(const void *addr, size_t size) {
    if (!kasan_initialized || size == 0 || addr == NULL) return;
...
void kasan_check_range(const void *addr, size_t size, bool is_write, const char *reason) {
    if (!kasan_is_active() || size == 0 || addr == NULL) return;
    uintptr_t start = (uintptr_t)addr;
    uintptr_t end   = start + size;
    if (end < start) {
