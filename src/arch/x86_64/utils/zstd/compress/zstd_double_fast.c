...
void ZSTD_fillDoubleHashTable(ZSTD_matchState_t *ms, void const *end,
    ...
    for (; ip + fastHashFillStep - 1 <= iend; ip += fastHashFillStep) {
        ...
        for (i = 0; i < fastHashFillStep; ++i) {
            ...
            if (i == 0) hashSmall[smHash] = curr + i;
            if (i == 0 || hashLarge[lgHash] == 0) hashLarge[lgHash] = curr + i;
...
size_t ZSTD_compressBlock_doubleFast_noDict_generic(ZSTD_matchState_t *ms, seqStore_t *seqStore,
                                                    U32 rep[ZSTD_REP_NUM], void const *src,
                                                    size_t srcSize, U32 const mls /* template */) {
    ...
    while (1) {
        ...
        do {
            ...
            if ((offset_1 > 0) & (MEM_read32(ip + 1 - offset_1) == MEM_read32(ip + 1))) {
                ...
                ip++;
                ZSTD_storeSeq(seqStore, (size_t)(ip - anchor), anchor, iend, STORE_REPCODE_1,
                              mLength);
                goto _match_stored;
            }
            ...
            if (idxs0 > prefixLowestIndex) {
                ...
                if (MEM_read32(matchs0) == MEM_read32(ip)) { goto _search_next_long; }
            }
...
FORCE_INLINE_TEMPLATE
size_t ZSTD_compressBlock_doubleFast_dictMatchState_generic(ZSTD_matchState_t *ms,
                                                            seqStore_t        *seqStore,
                                                            U32 rep[ZSTD_REP_NUM], void const *src,
                                                            size_t    srcSize,
    ...
    while (ip < ilimit) { /* < instead of <=, because repcode check at (ip+1) */
        ...
        if (((U32)((prefixLowestIndex - 1) - repIndex) >= 3 /* intentional underflow */) &&
            ...
            mLength =
                ZSTD_count_2segments(ip + 1 + 4, repMatch + 4, iend, repMatchEnd, prefixLowest) + 4;
            ip++;
        ...
        if (matchIndexL > prefixLowestIndex) {
            ...
            if (dictMatchL > dictStart && MEM_read64(dictMatchL) == MEM_read64(ip)) {
                mLength =
                    ZSTD_count_2segments(ip + 8, dictMatchL + 8, iend, dictEnd, prefixLowest) + 8;
...

    {
        ...
        if (matchIndexL3 > prefixLowestIndex) {
            ...
            if (dictMatchL3 > dictStart && MEM_read64(dictMatchL3) == MEM_read64(ip + 1)) {
                mLength =
                    ZSTD_count_2segments(ip + 1 + 8, dictMatchL3 + 8, iend, dictEnd, prefixLowest) +
                    8;
                ip++;
...
#define ZSTD_GEN_DFAST_FN(dictMode, mls)                                                           \
    static size_t ZSTD_compressBlock_doubleFast_##dictMode##_##mls(                                \
        ...
        size_t srcSize) {                                                                          \
        return ZSTD_compressBlock_doubleFast_##dictMode##_generic(ms, seqStore, rep, src, srcSize, \
                                                                  mls);                            \
    }
...
size_t ZSTD_compressBlock_doubleFast(ZSTD_matchState_t *ms, seqStore_t *seqStore,
                                     U32 rep[ZSTD_REP_NUM], void const *src, size_t srcSize) {
    const U32 mls = ms->cParams.minMatch;
    switch (mls) {
    default: /* includes case 3 */
    case 4: return ZSTD_compressBlock_doubleFast_noDict_4(ms, seqStore, rep, src, srcSize);
    case 5: return ZSTD_compressBlock_doubleFast_noDict_5(ms, seqStore, rep, src, srcSize);
    case 6: return ZSTD_compressBlock_doubleFast_noDict_6(ms, seqStore, rep, src, srcSize);
    case 7: return ZSTD_compressBlock_doubleFast_noDict_7(ms, seqStore, rep, src, srcSize);
    }
}

size_t ZSTD_compressBlock_doubleFast_dictMatchState(ZSTD_matchState_t *ms, seqStore_t *seqStore,
                                                    U32 rep[ZSTD_REP_NUM], void const *src,
                                                    size_t srcSize) {
    const U32 mls = ms->cParams.minMatch;
    switch (mls) {
    ...
    while (ip < ilimit) { /* < instead of <=, because (ip+1) */
        ...
        if ((((U32)((prefixStartIndex - 1) - repIndex) >=
            ...
            if ((matchLongIndex > dictStartIndex) && (MEM_read64(matchLong) == MEM_read64(ip))) {
                ...
                U32 offset;
                mLength =
                    ZSTD_count_2segments(ip + 8, matchLong + 8, iend, matchEnd, prefixStart) + 8;
                ...
                offset_2 = offset_1;
                offset_1 = offset;
                ZSTD_storeSeq(seqStore, (size_t)(ip - anchor), anchor, iend, STORE_OFFSET(offset),
                              mLength);
                ...
                if ((matchIndex3 > dictStartIndex) && (MEM_read64(match3) == MEM_read64(ip + 1))) {
                    ...
                    mLength =
                        ZSTD_count_2segments(ip + 9, match3 + 8, iend, matchEnd, prefixStart) + 8;
                    ip++;
                    ...
                    mLength =
                        ZSTD_count_2segments(ip + 4, match + 4, iend, matchEnd, prefixStart) + 4;
...
size_t ZSTD_compressBlock_doubleFast_extDict(ZSTD_matchState_t *ms, seqStore_t *seqStore,
                                             U32 rep[ZSTD_REP_NUM], void const *src,
                                             size_t srcSize) {
