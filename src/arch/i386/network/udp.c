...
uint8_t *UDP_Packet(uint16_t dest_port, uint16_t src_port, uint8_t *data, uint32_t size) {
    struct UDPMessage *res = (struct UDPMessage *)kmalloc(size + sizeof(struct UDPMessage));
...
void udp_handler(void *base) {
    ...
    if (swap16(udp->srcPort) == DHCP_SERVER_PORT) { // DHCP给我们分配IP
        ...
        struct Socket *socket =
            Socket_Find(swap32(ipv4->srcIP), swap16(udp->srcPort), swap32(ipv4->dstIP),
                        swap16(udp->dstPort), UDP_PROTOCOL);
        if (socket == -1) { return; }
