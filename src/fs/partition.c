...
static partition_t partitions[MAX_PARTITIONS_NUM];
static size_t partition_num = 0;

static void format_guid(const uint8_t guid[16], char out[37]) {
    snprintf(
...
size_t partition_read(void *handle, uint8_t *buf, size_t number, size_t lba) {
    ...
    return partition->device->ops.read(
        partition->device->handle, buf, number, partition->starting_lba + lba);
}

size_t partition_write(void *handle, uint8_t *buf, size_t number, uint64_t lba) {
    partition_t *partition = handle;
    return partition->device->ops.write(
        partition->device->handle, buf, number, partition->starting_lba + lba);
}
...
static bool parse_gpt_partitions(blk_device_t *disk, struct GPT_DPT *gpt) {
    ...
    if (blk_device_read(
            disk, (uint8_t *)dptes, gpt->partition_entry_lba * disk->block_size, dptes_size)
        == (size_t)-1) {
        free(dptes);
        return false;
    }
...
static bool parse_gpt_from_protective_mbr(blk_device_t *disk, uint8_t *mbr) {
    ...
    if (blk_device_read(
            disk, (uint8_t *)gpt, (uint64_t)gpt_lba_start * disk->block_size, disk->block_size)
        == (size_t)-1) {
        free(gpt);
        return false;
    }
