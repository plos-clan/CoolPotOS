...
errno_t procfs_mount(const char *src, vfs_node_t node) {
    ...
    vfs_node_t meminfo          = vfs_node_alloc(procfs_root, "meminfo");
    meminfo->type               = file_none;
    meminfo->mode               = 0700;
    proc_handle_t *meminfo_h    = malloc(sizeof(proc_handle_t));
    meminfo->handle             = meminfo_h;
    meminfo_h->task             = NULL;
    sprintf(meminfo_h->name, "meminfo");
