...
int create_user_process(const char *path, const char *cmdline, char *name, uint8_t level) {
    ...
    new_task->program_break = new_task->program_break_end = (void *) USER_AREA_START;//初始化程序断点和结束地址
    new_task->now_tid = 0;//初始化当前线程ID
    new_task->fpu_flag = 0;//初始化FPU标志
    new_task->status = RUNNING;//设置进程状态
    for (int i = 0; i < 255; i++) new_task->file_table[i] = NULL;//初始化文件表
    // 映射形参数据区
    ...
    for (uint32_t i = USER_STACK_TOP - STACK_SIZE; i < USER_STACK_TOP; i += PAGE_SIZE) {// 映射用户栈区
        alloc_frame(get_page(i, 1, new_task->pgd_dir), 0, 1);
    }
