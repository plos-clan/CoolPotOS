...
bool mod_check_signature(const uint8_t *module_buffer, size_t module_size) {
#if !(MODULE_CHECK)
    return true;
#endif
    ...
    if (sig_info->sig_len != ECC_SIG_LEN) {
        kerror("invalid signature length: %u. Expected %u for ECC P-256.", sig_info->sig_len, ECC_SIG_LEN);
        return false;
    }
    ...
    if (result == 1) {
        kinfo("MODCHK: Signature verification successful. Module is trusted.");
        return true;
    } else {
        kerror("MODCHK: Signature verification FAILED (Error %d). Untrusted module.", result);
        return false;
    }
}