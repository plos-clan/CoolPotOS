...
static const uint16_t ps2_to_linux[128] = {
    [0] = KEY_RESERVED,
    [1] = KEY_ESC,       [2] = KEY_1,         [3] = KEY_2,         [4] = KEY_3,
    [5] = KEY_4,         [6] = KEY_5,         [7] = KEY_6,         [8] = KEY_7,
    [9] = KEY_8,         [10] = KEY_9,        [11] = KEY_0,        [12] = KEY_MINUS,
    [13] = KEY_EQUAL,    [14] = KEY_BACKSPACE,[15] = KEY_TAB,      [16] = KEY_Q,
    [17] = KEY_W,        [18] = KEY_E,        [19] = KEY_R,        [20] = KEY_T,
    [21] = KEY_Y,        [22] = KEY_U,        [23] = KEY_I,        [24] = KEY_O,
    ...
    [45] = KEY_X,        [46] = KEY_C,        [47] = KEY_V,        [48] = KEY_B,
    [49] = KEY_N,        [50] = KEY_M,        [51] = KEY_COMMA,    [52] = KEY_DOT,
    [53] = KEY_SLASH,    [54] = KEY_RIGHTSHIFT,[55] = KEY_KPASTERISK,[56] = KEY_LEFTALT,
    [57] = KEY_SPACE,    [58] = KEY_CAPSLOCK, [59] = KEY_F1,       [60] = KEY_F2,
    [61] = KEY_F3,       [62] = KEY_F4,       [63] = KEY_F5,       [64] = KEY_F6,
    [65] = KEY_F7,       [66] = KEY_F8,       [67] = KEY_F9,       [68] = KEY_F10,
    [69] = KEY_NUMLOCK,  [70] = KEY_SCROLLLOCK,[71] = KEY_KP7,     [72] = KEY_KP8,
    [73] = KEY_KP9,      [74] = KEY_KPMINUS,  [75] = KEY_KP4,      [76] = KEY_KP5,
    [77] = KEY_KP6,      [78] = KEY_KPPLUS,   [79] = KEY_KP1,      [80] = KEY_KP2,
    [81] = KEY_KP3,      [82] = KEY_KP0,      [83] = KEY_KPDOT,
...
static void evdev_input_handler(indev_t *device, intype type, uint64_t code, uint8_t value) {
    ...
    if (evdev_ctx.open_count <= 0) return;
    ...
    if (ev_value == 0 && !is_modifier_keycode(keycode)) return;
    ...
    for (;;) {
        ...
        if (evdev_ctx.count > 0) {
            while (evdev_ctx.count > 0 && read_count < max_events) {
                memcpy((uint8_t *)addr + read_count * ev_size, &evdev_ctx.buf[evdev_ctx.head],
                       ev_size);
...
static int evdev_poll(void *handle, size_t events) {
    int out = 0;
    spin_lock(evdev_ctx.lock);
    if ((events & EPOLLIN) && evdev_ctx.count > 0) out |= EPOLLIN;
    spin_unlock(evdev_ctx.lock);
    return out;
}
...
static errno_t evdev_ioctl(void *handle, size_t req, void *arg) {
    ...
    if (type != EVDEV_IOC_TYPE) return -ENOSYS;

    switch (nr) {
    case EVIOC_NR_GVERSION: {
        // EVIOCGVERSION: return evdev protocol version
        if (arg == NULL || sz < sizeof(int)) return -EINVAL;
        *(int *)arg = EV_VERSION;
        return 0;
    }
    case EVIOC_NR_GID: {
        // EVIOCGID: return input device ID
        if (arg == NULL || sz < sizeof(struct input_id)) return -EINVAL;
    ...
    case EVIOC_NR_GNAME: {
        ...
        if (arg == NULL || sz == 0) return -EINVAL;
        size_t copy = (len + 1 < sz) ? len + 1 : sz;
        memcpy(arg, name, copy);
        if (copy <= len) ((char *)arg)[copy - 1] = '\0';
        return (errno_t)copy;
    }
    case EVIOC_NR_GPHYS: {
...
static void evdev_open(void *parent, const char *path, vfs_node_t node) {
    ...
    if (dtmp) dtmp->device_handle = &evdev_ctx;
...
void evdev_setup(vfs_node_t dev_root) {
    ...
    create_device_node_ex(input_dir, "event0", device_stream, &evdev_ctx, 0,
                          evdev_open, evdev_close,
                          (vfs_ioctl_t)evdev_ioctl,
                          (vfs_read_t)evdev_read, (vfs_write_t)evdev_write, (vfs_poll_t)evdev_poll,
                          NULL, NULL);
