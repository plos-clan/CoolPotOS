...
void *cpkrnl_dma_alloc(size_t size, uint64_t *phys_addr) {
    ...
    if (addr) {
        if (phys_addr) *phys_addr = driver_virt_to_phys(addr);
        memset(addr, 0, size);
    }
    return addr;
}
...
static void nvme_dump_status(nvme_controller_t *ctrl) {
    ...
    g_nvme_platform_ops->log("  RDY=%d CFS=%d SHST=%d NSSRO=%d\n", !!(csts & NVME_CSTS_RDY),
                             !!(csts & NVME_CSTS_CFS), (csts >> 2) & 0x3, !!(csts & (1 << 4)));
    g_nvme_platform_ops->log("  EN=%d CSS=%d MPS=%d AMS=%d SHN=%d IOSQES=%d IOCQES=%d\n",
                             !!(cc & NVME_CC_ENABLE), (cc >> 4) & 0x7, (cc >> 7) & 0xF,
                             (cc >> 11) & 0x7, (cc >> 14) & 0x3, (cc >> 16) & 0xF,
                             (cc >> 20) & 0xF);
}
...
static int nvme_reset_controller(nvme_controller_t *ctrl) {
    ...
    if (cc & NVME_CC_ENABLE) {
        ...
        if (csts & NVME_CSTS_RDY) {
            ...
            if (nvme_wait_ready(ctrl, false, 5000) != 0) { return -1; }
        }
    }
...
static int nvme_enable_controller(nvme_controller_t *ctrl) {
    ...
    if (cc_read != cc) {
        g_nvme_platform_ops->log("NVMe: CC register write failed! Expected 0x%08x, got 0x%08x\n",
                                 cc, cc_read);
        return -1;
    }
...
static int nvme_init_queue(nvme_controller_t *ctrl, nvme_queue_t *queue, uint16_t queue_id,
    ...
    g_nvme_platform_ops->log("NVMe: SQ allocated at virt=%p phys=0x%llx size=%d\n", queue->sq,
    ...
    g_nvme_platform_ops->log("NVMe: CQ allocated at virt=%p phys=0x%llx size=%d\n", queue->cq,
    ...
    g_nvme_platform_ops->log("NVMe: Queue %d doorbells - SQ offset=0x%x CQ offset=0x%x\n", queue_id,
...
static int nvme_process_queue_completions(nvme_controller_t *ctrl, nvme_queue_t *queue) {
    ...
    while (1) {
        ...
        if (!success) {
            g_nvme_platform_ops->log("NVMe: Command failed - CID=%d Status=0x%04x (SC=%d SCT=%d)\n",
        ...
        if (cid < UINT16_MAX && ctrl->requests[cid]) {
            ...
            if (req->callback) { req->callback(req->ctx, success, cqe->dw0); }
...
static int nvme_admin_cmd_sync(nvme_controller_t *ctrl, nvme_sqe_t *cmd, uint32_t *result,
                               uint32_t timeout_ms) {
    ...
    g_nvme_platform_ops->log("NVMe: Submitting admin command - opcode=%d cid=%d cdw0=0x%08x\n",
    ...
    if (result) { *result = sync_ctx.result; }
...
static int nvme_identify_controller(nvme_controller_t *ctrl, nvme_identify_ctrl_t *id_ctrl) {
    ...
    if (ret == 0) { memcpy(id_ctrl, buffer, sizeof(nvme_identify_ctrl_t)); }
...
static int nvme_identify_namespace(nvme_controller_t *ctrl, uint32_t nsid,
    ...
    if (ret == 0) { memcpy(id_ns, buffer, sizeof(nvme_identify_ns_t)); }
...
static int nvme_create_io_cq(nvme_controller_t *ctrl, nvme_queue_t *queue) {
    ...
    g_nvme_platform_ops->log("NVMe: Creating I/O CQ %d (depth=%d, phys=0x%llx)\n", queue->queue_id,
...
static int nvme_create_io_sq(nvme_controller_t *ctrl, nvme_queue_t *queue) {
    ...
    g_nvme_platform_ops->log("NVMe: Creating I/O SQ %d (depth=%d, phys=0x%llx)\n", queue->queue_id,
...
static int nvme_check_pci_config(pci_device_t *device) {
    ...
    if (!device->bars[0].address || device->bars[0].size < 0x2000) {
        g_nvme_platform_ops->log("NVMe: Invalid BAR0 (addr=0x%llx size=0x%llx)\n",
...
static int nvme_init_prp_pool(nvme_controller_t *ctrl, uint32_t num_lists) {
    ...
    g_nvme_platform_ops->log("NVMe: PRP pool initialized: %u lists, phys=0x%llx\n", num_lists,
...
static nvme_prp_list_t *nvme_alloc_prp_list(nvme_controller_t *ctrl, uint64_t *phys_addr) {
    ...
    if (phys_addr) { *phys_addr = ctrl->prp_list_pool_phys + (idx * sizeof(nvme_prp_list_t)); }
...
// 设置 PRP 条目（核心优化函数）
static int nvme_setup_prp(nvme_controller_t *ctrl, nvme_sqe_t *cmd, uint64_t phys_addr,
                          uint32_t size) {
    if (size == 0) {
    ...
    for (uint32_t page = 1; page < num_pages; page++) {
        if (prp_idx >= NVME_MAX_PRP_LIST_ENTRIES) {
            g_nvme_platform_ops->log("NVMe: PRP list overflow! pages=%u max_entries=%u\n",
                                     num_pages, NVME_MAX_PRP_LIST_ENTRIES);
            return -1;
        }
...
// 优化的异步读取（支持 PRP）
int nvme_read_async(nvme_controller_t *ctrl, uint32_t nsid, uint64_t lba, uint32_t block_count,
                    void *buffer, uint64_t buffer_phys, nvme_io_callback_t callback, void *ctx) {
...
// 优化的异步写入（支持 PRP）
int nvme_write_async(nvme_controller_t *ctrl, uint32_t nsid, uint64_t lba, uint32_t block_count,
                     const void *buffer, uint64_t buffer_phys, nvme_io_callback_t callback,
                     void *ctx) {
    if (!ctrl->initialized || nsid == 0 || nsid > ctrl->num_namespaces) {
        g_nvme_platform_ops->log("NVMe: Invalid namespace %u\n", nsid);
        return -1;
...
void nvme_probe(pci_device_t *device) {
    ...
    page_map_range(get_kernel_pagedir(), (uint64_t)ctrl->bar0, device->bars[0].address,
                   device->bars[0].size, KERNEL_PTE_FLAGS);
    ...
    if (nvme_init_prp_pool(ctrl, num_prp_lists) != 0) { goto error; }
