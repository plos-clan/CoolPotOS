...
static int plainfb_destroy_dumb(drm_device_t *drm_dev, uint32_t handle) {
    ...
    if (handle >= 32 || !gpu_dev->dumbbuffers[handle].used) { return -EINVAL; }

    if (--gpu_dev->dumbbuffers[handle].refcount == 0) {
        // Free memory
        free_frames(gpu_dev->dumbbuffers[handle].addr,
                    (gpu_dev->dumbbuffers[handle].pitch * gpu_dev->dumbbuffers[handle].height +
                     PAGE_SIZE - 1) /
                        PAGE_SIZE);
...
int plainfb_map_dumb(drm_device_t *drm_dev, struct drm_mode_map_dumb *args) {
    ...
    if (args->handle >= 32 || !gpu_dev->dumbbuffers[args->handle].used) { return -EINVAL; }
...
static int plainfb_page_flip(drm_device_t *drm_dev, struct drm_mode_crtc_page_flip *flip) {
    ...
    memcpy((void *)gpu_dev->framebuffer->address,
...
static int plainfb_get_connectors(drm_device_t *drm_dev, drm_connector_t **connectors,
                                  uint32_t *count) {
    plainfb_device_t *gpu_dev = drm_dev->data;
    *count                    = 0;

    for (uint32_t i = 0; i < 1; i++) {
        if (gpu_dev->connectors[i]) { connectors[(*count)++] = gpu_dev->connectors[i]; }
    }
...
static int plainfb_get_crtcs(drm_device_t *drm_dev, drm_crtc_t **crtcs, uint32_t *count) {
    ...
    *count                    = 0;
    ...
    for (uint32_t i = 0; i < 1; i++) {
        if (gpu_dev->crtcs[i]) { crtcs[(*count)++] = gpu_dev->crtcs[i]; }
    }
...
static int plainfb_get_encoders(drm_device_t *drm_dev, drm_encoder_t **encoders, uint32_t *count) {
    ...
    *count                    = 0;
    ...
    for (uint32_t i = 0; i < 1; i++) {
        if (gpu_dev->encoders[i]) { encoders[(*count)++] = gpu_dev->encoders[i]; }
    }
