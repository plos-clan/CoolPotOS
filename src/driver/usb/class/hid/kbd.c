...
static void keyboard_handle_completion(UsbDriver *driver, CompletionEvent event) {
    ...
    if (event.ep_addr != kbd->hid.ep_addr) { return; }
    ...
    for (size_t i = 0; i < kbd->layout.modifiers.len; i++) {
        ...
        if (hid_field_value(mod, data, 0) == 1) {
            ...
            if (usage < 256) { now_pressed[usage] = true; }
            if (usage == 0xE1 || usage == 0xE5) { shift = true; }
            if (usage == 0xE0 || usage == 0xE4) { ctrl = true; }
        }
    }
    ...
    for (size_t i = 0; i < kbd->layout.arrays.len; i++) {
        ...
        for (uint32_t j = 0; j < arr->report_count; j++) {
            ...
            if (usage > 1 && usage < 256) { now_pressed[usage] = true; }
        }
    }
    ...
    for (uint16_t usage = 0; usage < 256; usage++) {
        if (now_pressed[usage] && !kbd->pressed[usage]) {
            if (kbd->input_dev) { send_input_event(kbd->input_dev, EV_KEY, usage, EV_PRESS); }
            ...
            if (kbd->input_dev) { send_input_event(kbd->input_dev, EV_KEY, usage, EV_RELEASE); }
        }
    }
...
static void keyboard_scan_layout(Keyboard *kbd) {
    for (uint32_t i = 0; i < 256; i++) {
        if (!kbd->hid.descriptor.reports.used[i]) { continue; }
        HidReport *report = &kbd->hid.descriptor.reports.values[i];
        if (hid_report_size_bytes(report, HID_KIND_INPUT) == 0) { continue; }
        for (size_t j = 0; j < report->fields.len; j++) {
            HidField *field = &report->fields.data[j];
            if (field->kind != HID_KIND_INPUT) { continue; }
            if (hid_field_is_const(field) || field->usage_page != 0x07) { continue; }
...
UsbDriver *hid_probe_kbd(UsbInterface *iface) {
    if (!usb_interface_matches(iface, USB_CLASS_HID, 1, 1)) { return NULL; }
