...
static uint32_t xhci_setup_one_endpoint(Xhci *xhci, uint8_t slot_id, uint64_t ctx_base,
                                       UsbEndpoint *ep) {
...
bool xhci_configure_endpoints(Xhci *xhci, uint8_t slot_id, UsbEndpointVec *endpoints) {
    ...
    for (size_t i = 0; i < endpoints->len; i++) {
        uint32_t dci = xhci_setup_one_endpoint(xhci, slot_id, (uint64_t)(uintptr_t)in_ctx_virt,
                                               &endpoints->data[i]);
        if (dci > max_dci) {
            max_dci = dci;
        }
