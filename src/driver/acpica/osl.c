...
ACPI_CPU_FLAGS AcpiOsAcquireLock(ACPI_SPINLOCK Handle) {
    if (Handle) { spin_lock(*(spin_t *)Handle); }
    return 0;
}

void AcpiOsReleaseLock(ACPI_SPINLOCK Handle, ACPI_CPU_FLAGS Flags) {
    (void)Flags;
    if (Handle) { spin_unlock(*(spin_t *)Handle); }
}
...
ACPI_STATUS AcpiOsWaitSemaphore(ACPI_SEMAPHORE Handle, UINT32 Units, UINT16 Timeout) {
    ...
    if (!sem || Units == 0) { return AE_BAD_PARAMETER; }
    ...
    while (Units--) {
        ...
        if (ns > UINT32_MAX) { ns = UINT32_MAX; }
        if (!sem_wait(sem, (uint32_t)ns)) { return AE_TIME; }
    }
    return AE_OK;
}
...
void *AcpiOsMapMemory(ACPI_PHYSICAL_ADDRESS Where, ACPI_SIZE Length) {
    ...
    page_map_range(get_kernel_pagedir(), (uint64_t)vaddr & ~(PAGE_SIZE - 1),
                   Where & ~(PAGE_SIZE - 1), Length, KERNEL_PTE_FLAGS);
    return vaddr;
}
...
ACPI_STATUS AcpiOsInstallInterruptHandler(UINT32 InterruptNumber, ACPI_OSD_HANDLER ServiceRoutine,
    ...
    if (!arg) { return AE_NO_MEMORY; }
    arg->irq_handler              = ServiceRoutine;
    arg->ctx                      = Context;
#if defined(__x86_64__) || defined(__amd64__)
    extern intctl_t apic_controller;
    irq_regist_irq(InterruptNumber + IRQ_BASE_VECTOR, acpica_irq_handler, InterruptNumber, arg,
...
ACPI_STATUS AcpiOsRemoveInterruptHandler(UINT32 InterruptNumber, ACPI_OSD_HANDLER ServiceRoutine) {
    ...
    if (!ServiceRoutine) { return AE_BAD_PARAMETER; }
    if (vector >= ARCH_MAX_IRQ_NUM) { return AE_BAD_PARAMETER; }

    action = &actions[vector];
    if (!action->handler) { return AE_NOT_EXIST; }
...
ACPI_STATUS AcpiOsReadPort(ACPI_IO_ADDRESS Address, UINT32 *Value, UINT32 Width) {
    ...
    switch (Width) {
    case 8: *Value = io_in8((uint16_t)Address); return AE_OK;
    case 16: *Value = io_in16((uint16_t)Address); return AE_OK;
    case 32: *Value = io_in32((uint16_t)Address); return AE_OK;
...
#ifdef __x86_64__
    switch (Width) {
    case 8: io_out8((uint16_t)Address, (uint8_t)Value); return AE_OK;
    case 16: io_out16((uint16_t)Address, (uint16_t)Value); return AE_OK;
    case 32: io_out32((uint16_t)Address, (uint32_t)Value); return AE_OK;
    default: return AE_BAD_PARAMETER;
    }
#else
    (void)Address;
...
ACPI_STATUS AcpiOsReadMemory(ACPI_PHYSICAL_ADDRESS Address, UINT64 *Value, UINT32 Width) {
    ...
    if (!ptr || !Value) { return AE_BAD_PARAMETER; }

    switch (Width) {
    case 8: *Value = *(volatile uint8_t *)ptr; return AE_OK;
    case 16: *Value = *(volatile uint16_t *)ptr; return AE_OK;
    case 32: *Value = *(volatile uint32_t *)ptr; return AE_OK;
    case 64: *Value = *(volatile uint64_t *)ptr; return AE_OK;
    default: return AE_BAD_PARAMETER;
    }
}
...
ACPI_STATUS AcpiOsWriteMemory(ACPI_PHYSICAL_ADDRESS Address, UINT64 Value, UINT32 Width) {
    ...
    if (!ptr) { return AE_BAD_PARAMETER; }

    switch (Width) {
    case 8: *(volatile uint8_t *)ptr = (uint8_t)Value; return AE_OK;
    case 16: *(volatile uint16_t *)ptr = (uint16_t)Value; return AE_OK;
    case 32: *(volatile uint32_t *)ptr = (uint32_t)Value; return AE_OK;
    case 64: *(volatile uint64_t *)ptr = (uint64_t)Value; return AE_OK;
    default: return AE_BAD_PARAMETER;
    }
}
...
ACPI_STATUS AcpiOsReadPciConfiguration(ACPI_PCI_ID *PciId, UINT32 Reg, UINT64 *Value,
    ...
    switch (Width) {
    case 8: *Value = (data >> ((Reg & 3U) * 8U)) & 0xFFU; break;
    case 16: *Value = (data >> ((Reg & 2U) * 8U)) & 0xFFFFU; break;
...
ACPI_STATUS AcpiOsWritePciConfiguration(ACPI_PCI_ID *PciId, UINT32 Reg, UINT64 Value,
    ...
    case 16:
        ...
        break;
    case 32: data = (uint32_t)Value; break;
    case 64:
        pci_write(PciId->Bus, PciId->Device, PciId->Function, PciId->Segment, (Reg & ~0x3U) + 4,
                  (uint32_t)(Value >> 32));
        data = (uint32_t)Value;
        break;
    }
...
ACPI_STATUS AcpiOsSignal(UINT32 Function, void *Info) {
    (void)Info;
    if (Function == ACPI_SIGNAL_FATAL) { logkf("acpica: fatal signal\n"); }
    return AE_OK;
}
...
ACPI_STATUS AcpiOsGetLine(char *Buffer, UINT32 BufferLength, UINT32 *BytesRead) {
    ...
    if (!Buffer || !BytesRead || BufferLength == 0) { return AE_BAD_PARAMETER; }
    if (!current_session) { return AE_NOT_IMPLEMENTED; }
...
ACPI_STATUS AcpiOsGetTableByName(char *Signature, UINT32 Instance, ACPI_TABLE_HEADER **Table,
                                 ACPI_PHYSICAL_ADDRESS *Address) {
    ACPI_STATUS st;

    if (!Signature || !Table) { return AE_BAD_PARAMETER; }
...
ACPI_STATUS AcpiOsGetTableByIndex(UINT32 Index, ACPI_TABLE_HEADER **Table, UINT32 *Instance,
                                  ACPI_PHYSICAL_ADDRESS *Address) {
    ACPI_STATUS      st;
    ACPI_TABLE_HEADER *hdr;
    UINT32           inst = 1;

    if (!Table) { return AE_BAD_PARAMETER; }

    st = AcpiGetTableByIndex(Index, &hdr);
    if (ACPI_FAILURE(st)) { return st; }
    ...
    if (Address) { *Address = (ACPI_PHYSICAL_ADDRESS)virt_to_phys(hdr); }
    *Table = hdr;
    return AE_OK;
}
