...
static void *fb_dev_map(void *data, void *addr, size_t offset, size_t size, size_t prot,
                        size_t flags) {
    boot_framebuffer_t *framebuffer = (boot_framebuffer_t *)data;
    ...
    page_map_range(get_current_directory(), (uint64_t)addr, fb_addr,
                   framebuffer->width * framebuffer->height * framebuffer->bpp / 8, page_flags);
    return addr;
}

void fb_setup(vfs_node_t dev_root) {
    if (boot_framebuffer_count() == 0) return;
    boot_framebuffer_t *fb = boot_get_framebuffer(0);
    if (fb == NULL || dev_root == NULL) return;

    uint64_t dev_number = ((uint64_t)FB_MAJOR << 8) | 0;
    create_device_node(dev_root, "fb0", device_stream, fb, dev_number, (vfs_ioctl_t)fb_dev_ioctl,
