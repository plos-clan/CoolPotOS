...
int hba_prepare_cmd(struct hba_port *port, struct hba_cmdt **cmdt, struct hba_cmdh **cmdh) {
    ...
    page_map_to(get_kernel_pagedir(), (uint64_t)cmd_table, phys, KERNEL_PTE_FLAGS);
...
int hba_bind_vbuf(struct hba_cmdh *cmdh, struct hba_cmdt *cmdt, struct vecbuf *vbuf) {
    ...
    do {
        cmdt->entries[i++] = (struct hba_prdte){.data_base  = virt_to_phys(pos->buf.buffer),
...
int ahci_init_device(struct hba_port *port) {
    ...
    page_map_range(get_kernel_pagedir(), (uint64_t)data, (uint64_t)data_in, PAGE_SIZE,
                   KERNEL_PTE_FLAGS);
    ...
    free_frame(virt_to_phys(cmd_table));
    return 1;
...
void load_ahci(pci_device_t *device) {
    if (device->bars[5].address == 0 || device->bars[5].size == 0) {
    ...
    not_null_assert(hba, "Out of memory for AHCI HBA structure");
    memset(hba, 0, sizeof(struct ahci_hba));

    hba->base = (hba_reg_t *)phys_to_virt(device->bars[5].address);
    page_map_range(get_kernel_pagedir(), (uint64_t)hba->base, device->bars[5].address,
                   device->bars[5].size, KERNEL_PTE_FLAGS);
...
void ahci_setup() {
    pci_find_class(0x010601, load_ahci);
}
