...
int hba_bind_sbuf(struct hba_cmdh *cmdh, struct hba_cmdt *cmdt, void *buf, uint32_t len) {
    ...
    uint64_t buf_phys = driver_virt_to_phys(buf);
    if (buf_phys == 0) {
...
void load_ahci(pci_device_t *device) {
    ...
    for (size_t i = 0, fisp = 0, clbp = 0; i < 32;
        ...
        if (!clbp) {
            ...
            page_map_range(
                get_kernel_pagedir(), (uint64_t)vaddr, clb_pa, PAGE_SIZE, KERNEL_PTE_FLAGS
            );
        ...
        if (!fisp) {
            ...
            page_map_range(
                get_kernel_pagedir(), (uint64_t)vaddr, fis_pa, PAGE_SIZE, KERNEL_PTE_FLAGS
            );
            memset(vaddr, 0, 0x1000);
        }
