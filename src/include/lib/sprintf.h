...
#    include "types/stdarg.h" // for va_arg(), va_list()
#    include "types/stddef.h" // size_t, ptrdiff_t
...
STBSP__PUBLICDEC int STB_SPRINTF_DECORATE(vsnprintf)(char *buf, int count, char const *fmt,
                                                     va_list va);
...
STBSP__PUBLICDEC int  STB_SPRINTF_DECORATE(vsprintfcb)(STBSP_SPRINTFCB *callback, void *user,
...
static stbsp__int32 stbsp__real_to_str(char const **start, stbsp__uint32 *len, char *out,
                                       stbsp__int32 *decimal_pos, double value,
...
STBSP__PUBLICDEF int STB_SPRINTF_DECORATE(vsprintfcb)(STBSP_SPRINTFCB *callback, void *user,
                                                      char *buf, char const *fmt, va_list va) {
...
STBSP__PUBLICDEF int STB_SPRINTF_DECORATE(vsnprintf)(char *buf, int count, char const *fmt,
                                                     va_list va) {
    stbsp__context c;
...
static stbsp__int32 stbsp__real_to_str(char const **start, stbsp__uint32 *len, char *out,
    ...
    frac_digits =
        (frac_digits & 0x80000000) ? ((frac_digits & 0x7ffffff) + 1) : (tens + frac_digits);
    if ((frac_digits < 24)) {
        stbsp__uint32 dg = 1;
