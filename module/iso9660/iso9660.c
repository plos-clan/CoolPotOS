...
int iso9660_id = -1;
errno_t iso9660_mkdir(void *parent, const char *name, vfs_node_t node) {
    return -EROFS;
}

errno_t iso9660_mkfile(void *parent, const char *name, vfs_node_t node) {
    return -EROFS;
}
...
static struct vfs_callback iso_callbacks = {
    ...
    .stat     = iso9660_stat,
    .ioctl    = (void *)dummy,
    .dup      = (void *)dummy, //TODO 未实现
    .delete   = (void *)dummy, // 只读文件系统
    .rename   = (void *)dummy, // 只读文件系统
    .poll     = (void *)dummy, // 只读文件系统
    .map      = (void *)dummy, //TODO 未实现
    .free     = (void *)dummy, // 只读文件系统
    .chmod    = (void *)dummy, // 只读文件系统,
    .mknod    = (void *)dummy, // 只读文件系统
};
