...
static void eap_figure_next_state(ppp_pcb *pcb, int status) {
    ...
    case eapIdentify:
        ...
        if (pcb->eap.es_server.ea_peerlen > SRP_PSEUDO_LEN &&
            ...
            BZERO(&bs, sizeof(bs));
            plen =
                b64dec(&bs, pcb->eap.es_server.ea_peer + SRP_PSEUDO_LEN,
                       pcb->eap.es_server.ea_peerlen - SRP_PSEUDO_LEN, secbuf);
            toffs = 0;
            for (i = 0; i < 5; i++) {
                pncrypt_setkey(toffs);
                toffs -= 86400;
                /* FIXME: if we want to do SRP, we need to find a way to pass
                 * the PolarSSL des_context instead of using static memory */
            ...
            if (plen % 8 == 0 && i < 5) {
                ...
                sp = secbuf + 8;
                while (plen > 0) {
                    /* FIXME: if we want to do SRP, we need to find a way to
                     * pass the PolarSSL des_context instead of using static
                     * memory */
        ...
        if (get_srp_secret(pcb->eap.es_unit, pcb->eap.es_server.ea_peer,
                           pcb->eap.es_server.ea_name, (char *)secbuf,
                           1) != 0) {
...
u_char *str;
{
    ...
    msglen =
        EAP_HEADERLEN + 2 * sizeof(u_char) + sizeof(u32_t) + SHA_DIGESTSIZE;
    p = pbuf_alloc(PBUF_RAW, (u16_t)(PPP_HDRLEN + msglen), PBUF_RAM);
    if (NULL == p)
        return;
    if (p->tot_len != p->len) {
...
static void eap_request(ppp_pcb *pcb, u_char *inp, int id, int len) {
    ...
    case EAPT_MD5CHAP:
        ...
        if (vallen < 8 || vallen > len) {
            ppp_error(
                ("EAP: MD5-Challenge with bad length %d (8..%d)", vallen, len));
    ...
    case EAPT_SRP:
        ...
        case EAPSRP_CHALLENGE:
            ...
            if (tc == NULL) {
                ...
                tc = t_clientopen(pcb->eap.es_client.ea_name, &Nval, &gval,
                                  &sval);
                if (tc == NULL) {
        ...
        case EAPSRP_SKEY:
            ...
            if (pcb->eap.es_client.ea_skey != NULL) {
                ...
                if (get_srp_secret(pcb->eap.es_unit, pcb->eap.es_client.ea_name,
                                   pcb->eap.es_client.ea_peer, secret,
                                   0) == 0) {
                    /*
