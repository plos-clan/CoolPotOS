...
static inline void ext4_xattr_compute_hash(struct ext4_xattr_header *header,
    ...
    if (entry->e_value_block == 0 && entry->e_value_size != 0) {
        ...
        for (n = (to_le32(entry->e_value_size) + EXT4_XATTR_ROUND) >> EXT4_XATTR_PAD_BITS; n; n--) {
            hash = (hash << VALUE_HASH_SHIFT) ^ (hash >> (8 * sizeof(hash) - VALUE_HASH_SHIFT)) ^
                   to_le32(*value++);
        }
    }
...
static void ext4_xattr_rehash(struct ext4_xattr_header *header, struct ext4_xattr_entry *entry) {
    ...
    while (!EXT4_XATTR_IS_LAST_ENTRY(here)) {
        ...
        hash = (hash << BLOCK_HASH_SHIFT) ^ (hash >> (8 * sizeof(hash) - BLOCK_HASH_SHIFT)) ^
               to_le32(here->e_hash);
...
static void ext4_xattr_set_block_checksum(struct ext4_inode_ref    *inode_ref,
                                          ...
                                          struct ext4_xattr_header *header) {
    struct ext4_sblock *sb = &inode_ref->fs->sb;
    if (!ext4_sb_feature_ro_com(sb, EXT4_FRO_COM_METADATA_CSUM)) return;
    ...
    for (i = 0; prefix_tbl[i].prefix; i++) {
        ...
        if (full_name_len >= prefix_len && !memcmp(full_name, prefix_tbl[i].prefix, prefix_len)) {
            ...
            *name_index       = prefix_tbl[i].name_index;
            if (name_len) *name_len = full_name_len - prefix_len;

            if (!(full_name_len - prefix_len) && require_name) return NULL;

            *found = true;
            if (require_name) return full_name + prefix_len;
...
const char *ext4_get_xattr_name_prefix(uint8_t name_index, size_t *ret_prefix_len) {
    ...
    for (i = 0; prefix_tbl[i].prefix; i++) {
        ...
        if (prefix_tbl[i].name_index == name_index) {
            if (ret_prefix_len) *ret_prefix_len = prefix_len;
 ...
 */
static int ext4_xattr_set_entry(struct ext4_xattr_info *i, struct ext4_xattr_search *s,
                                bool dry_run) {
	 ...
	 * indicate success.
	 */
    if (!i->value && s->not_found) return EOK;
    ...
    for (; !EXT4_XATTR_IS_LAST_ENTRY(last); last = EXT4_XATTR_NEXT(last)) {
        if (last->e_value_size) {
            ...
            if (offs < min_offs) min_offs = offs;
        }
    }
    ...
    if (i->value) {
        ...
        if (free < EXT4_XATTR_SIZE(i->value_len) + EXT4_XATTR_LEN(name_len)) return ENOSPC;
    }

    /* Return EOK now if we do not intend to modify the content. */
    if (dry_run) return EOK;
    ...
    if (!s->not_found) {
        ...
        for (last = s->first; !EXT4_XATTR_IS_LAST_ENTRY(last); last = EXT4_XATTR_NEXT(last)) {
            ...
            if (offs < value_offs) last->e_value_offs = to_le16(offs + value_size);
        }
    }
    ...
    if (i->value) {
        ...
        if (value_offs) {
            memcpy((char *)s->base + value_offs, (char *)i->value, i->value_len);
...
static inline bool ext4_xattr_is_empty(struct ext4_xattr_search *s) {
    if (!EXT4_XATTR_IS_LAST_ENTRY(s->first)) return false;
...
static bool ext4_xattr_is_block_valid(struct ext4_inode_ref *inode_ref, struct ext4_block *block) {
	 ...
	 */
    if (header->h_magic != to_le32(EXT4_XATTR_MAGIC)) return false;

    /*
	 * The in-kernel filesystem driver only supports 1 block currently.
	 */
    if (header->h_blocks != to_le32(1)) return false;
    ...
    for (; !EXT4_XATTR_IS_LAST_ENTRY(entry); entry = EXT4_XATTR_NEXT(entry)) {
        if (!to_le32(entry->e_value_size) && to_le16(entry->e_value_offs)) return false;

        if ((char *)base + to_le16(entry->e_value_offs) + to_le32(entry->e_value_size) >
            (char *)end)
            return false;
		 ...
		 * end.
		 */
        if ((char *)EXT4_XATTR_NEXT(entry) + sizeof(uint32_t) > (char *)end) return false;
	 ...
	 */
    if ((char *)base + min_offs < (char *)entry + sizeof(uint32_t)) return false;
...
static bool ext4_xattr_is_ibody_valid(struct ext4_inode_ref *inode_ref) {
	 ...
	 * Check whether the magic number in the header is correct.
	 */
    if (iheader->h_magic != to_le32(EXT4_XATTR_MAGIC)) return false;
...
static void ext4_xattr_block_init_search(struct ext4_inode_ref    *inode_ref,
                                         struct ext4_xattr_search *s, struct ext4_block *block) {
    s->base      = block->data;
...
static int ext4_xattr_block_find_entry(struct ext4_inode_ref    *inode_ref,
                                       struct ext4_xattr_finder *finder, struct ext4_block *block) {
    int ret = EOK;
    ...
    if (!ext4_xattr_is_block_valid(inode_ref, block)) return EIO;
...
static int ext4_xattr_ibody_find_entry(struct ext4_inode_ref    *inode_ref,
    ...
    /* Check the validity of the buffer */
    if (!ext4_xattr_is_ibody_valid(inode_ref)) return EIO;
 ...
 * @return Error code
 */
int ext4_xattr_list(struct ext4_inode_ref *inode_ref, struct ext4_xattr_list_entry *list,
                    size_t *list_len) {
    int                             ret     = EOK;
    size_t                          buf_len = 0;
    struct ext4_fs                 *fs      = inode_ref->fs;
    ...
    if (extra_isize && ext4_xattr_is_ibody_valid(inode_ref)) {
        ...
        for (; !EXT4_XATTR_IS_LAST_ENTRY(entry); entry = EXT4_XATTR_NEXT(entry)) {
            ...
            if (list) {
                ...
                if (list_prev) list_prev->next = list;
    ...
    if (list_prev) list_prev->next = NULL;
out:
    if (ret == EOK && list_len) *list_len = buf_len;

    if (block_loaded) ext4_block_set(fs->bdev, &block);
 ...
 * @return Error code
 */
int ext4_xattr_get(struct ext4_inode_ref *inode_ref, uint8_t name_index, const char *name,
                   size_t name_len, void *buf, size_t buf_len, size_t *data_len) {
    int                      ret = EOK;
...
out:
    if (ret == EOK && data_len) *data_len = value_len;
...
static int ext4_xattr_copy_new_block(struct ext4_inode_ref *inode_ref, struct ext4_block *block,
                                     struct ext4_block *new_block, ext4_fsblk_t *orig_block,
                                     bool *allocated) {
    int                       ret         = EOK;
    ext4_fsblk_t              xattr_block = 0;
    struct ext4_xattr_header *header;
    ...
    if (orig_block) *orig_block = block->lb_id;
 ...
 * @return Error code
 */
int ext4_xattr_remove(struct ext4_inode_ref *inode_ref, uint8_t name_index, const char *name,
                      size_t name_len) {
    int                      ret = EOK;
    struct ext4_block        block;
    struct ext4_xattr_finder ibody_finder;
 ...
 * @return Error code
 */
static int ext4_xattr_block_set(struct ext4_inode_ref *inode_ref, struct ext4_xattr_info *i,
                                bool no_insert) {
    int               ret       = EOK;
    bool              allocated = false;
    struct ext4_fs   *fs        = inode_ref->fs;
    ...
    if (!orig_xattr_block) {
        ...
        ext4_block_set(fs->bdev, &block);
        if (ret != EOK) ext4_xattr_try_free_block(inode_ref);
    } else {
        struct ext4_xattr_finder  finder;
        struct ext4_xattr_header *header;
        finder.i = *i;
		 ...
		 */
        ret =
            ext4_xattr_copy_new_block(inode_ref, &block, &new_block, &orig_xattr_block, &allocated);
        if (ret != EOK) {
            ext4_block_set(fs->bdev, &block);
            goto out;
        }
 ...
 * @return Error code
 */
int ext4_xattr_set(struct ext4_inode_ref *inode_ref, uint8_t name_index, const char *name,
                   size_t name_len, const void *value, size_t value_len) {
    int                      ret = EOK;
    struct ext4_fs          *fs  = inode_ref->fs;
    struct ext4_xattr_finder ibody_finder;
    struct ext4_xattr_info   i;
    ...
    try_insert:
        ...
        if (ret == ENOSPC) {
            ...
            if (block_found) ret = ext4_xattr_block_remove(inode_ref, &i);
