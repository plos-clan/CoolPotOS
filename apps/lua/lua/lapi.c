...
static void auxsetstr(lua_State *L, const TValue *t, const char *k) {
    ...
    if (luaV_fastget(L, t, str, slot, luaH_getstr)) {
        ...
        api_incr_top(L);
        /*TODO nil*/luaV_finishset(L, t, s2v(L->top - 1), s2v(L->top - 2), slot);
...
LUA_API int lua_gc(lua_State *L, int what, ...) {
    ...
    switch (what) {
        ...
        case LUA_GCGEN: {
            int minormul = va_arg(argp,
            int);
            int majormul = va_arg(argp,
            int);
        ...
        case LUA_GCINC: {
            int pause = va_arg(argp,
            int);
            int stepmul = va_arg(argp,
            int);
            int stepsize = va_arg(argp,
            int);
            res = isdecGCmodegen(g) ? LUA_GCGEN : LUA_GCINC;
            if (pause != 0)
